        	...
        	for (int x = 0; x < image_width; x++) {
            	...
            	for (int dy = -maximum_displacement; dy <= maximum_displacement; dy++) {
                	for (int dx = -maximum_displacement; dx <= maximum_displacement; dx++) {
                    		if (y + dy - feature_height < 0 || y + dy + feature_height >= image_height
                            		|| x + dx - feature_width < 0 || x + dx + feature_width >= image_width) {
                        		continue;
                    		}
...
//the improvement:
					...
					for (int box_x =  2 * feature_width / 4 * 4 - feature_width; box_x <= feature_width; box_x++) {
                                                ...
                                                int right_x = x + dx + box_x;
                                                int right_y = y + dy + box_y;
                                                squared_diff += square_euclidean_distance(left[left_y * image_width + left_x],
                                                        right[right_y * image_width + right_x]);
                                        }


                    		}
                    		if (min_diff == -1 || min_diff > squared_diff || (min_diff == squared_diff
